...
int main()
{
    ...
    for(ll i=0;i<n;i++)
    {
        ...
        for(ll j=0;j<n;j++)
        {
            ...
            {
                m[INT_MAX][x[j]]++;
                ma=max(m[INT_MAX][x[j]],ma);
            }
            else
            {
                ld slope=(ld)ydiff/(ld)xdiff;
                ld x_cor=x[i]-y[i]*(x[i]-x[j])/((ld)(y[i]-y[j]));
                m[slope][x_cor]++;
                ma=max(m[slope][x_cor],ma);
            }
        }
        max_in_a_line=max(max_in_a_line,ma);
    }
    cout<<max_in_a_line+1<<endl;
}
